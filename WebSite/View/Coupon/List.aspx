    ...
    <script type="text/jquery-tmpl-x" id="j-tmpl-ticket-listitem">
        	{{each(i, v) list}}
	        	<tr data-gid="${v.id}">
					...
					<td><b class="text-error">${v.usertimes}</b>次</td>
					<td style="width:200px;">
						<a class="btn btn-primary btn-mini" href="edit.aspx?id=${v.id}&sellerid=${sellerId}"><i class="icon-pencil icon-white"></i> 编辑</a>
                        {{if v.enabled == 1}}
						<a class="btn btn-info btn-mini j-btn-pause" href=""><i class="icon-pause icon-white"></i> 下架</a>
                        {{else}}
                ...
                $("#j-btn-delSelected").bind("click", function () {
                    ...
                    if (ids.length > 0) {
                        Common.confirm({
                            ...
                            confirm: function () {
                                $.ajax({
                                    url: "../../Handler/Backstage/CouponHandler.ashx?action=delete&ids=" + ids,
                                    type: "Get",
                                    dataType: "json"
